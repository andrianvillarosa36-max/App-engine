...
That's it — push to `main` and check the **Actions** tab to watch the
workflow run. Once it finishes, your app is live at:

```
https://<your-username>.github.io/App-engine/
```

(`vite.config.js` has `base: "/App-engine/"` set to match — if you ever
rename the repo, update that value too, or asset paths will break.)

If you just pushed the workflow file itself from Termux, the first run
kicks off automatically — no extra command needed.
